...
import CreateJobCategory from "./CreateCategory";
import { listJobCategories, useListJobCategories } from "../../../kubb";
...
export interface JobCategoryValues {
  id: number;
  name?: string;
}

type TableData = Awaited<ReturnType<typeof listJobCategories>>["data"][number];

function JobCategoryList() {
  const INITIAL_FORM_DATA: JobCategoryValues = {
    id: 0,
    name: "",
  };
  ...
  const [initialFormData, setInitialFormData] = useState<JobCategoryValues>(INITIAL_FORM_DATA);

  const listItems = useListJobCategories({
    ...paginationModel,
  });
  ...
  const dialogCreateDriver = useMemo(
    () =>
      ViewEditDialog(CreateJobCategory)({
  ...
  const handleClickDriverCreateOpen = (dialogTitle: string, formState: JobCategoryValues) => {
    ...
    <Box width="100%">
      <Grid container justifyContent="center" spacing={2}>
                  ...
                  <Grid item>
                    <Button
                      ...
                      onClick={() => {
                        ...
                        handleClickDriverCreateOpen(`Add Job Category`, INITIAL_FORM_DATA);
                      }}
                      startIcon={<AddIcon />}
                    >
                      Create Job Category
        ...
        {openCreateDriver && !matchDownSM ? (
          <Grid item lg={5} md={5}>
            <MainCard title={dialogTitle}>
              <CreateJobCategory initialItem={initialFormData} setOpen={setOpenCreateDriver} fetchData={listItems.refetch} />
            </MainCard>
          </Grid>
        ) : (
          dialogCreateDriver
        ...
        {openUpdateDriver && !matchDownSM && (
          <Grid item lg={5} md={5}>
            <MainCard title={dialogTitle}>
              <CreateJobCategory initialItem={initialFormData} setOpen={setOpenUpdateDriver} fetchData={listItems.refetch} />
            </MainCard>
          </Grid>
        )}
      </Grid>
